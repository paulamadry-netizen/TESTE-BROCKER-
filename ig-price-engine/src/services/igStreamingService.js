...
class IGStreamingService {
   ...
   * @param {string} cst - CST token from REST API login
   * @param {string} xSecurityToken - X-SECURITY-TOKEN from REST API login
   ...
   * @param {string[]} epics - Array of epic identifiers
  ...
  subscribeToEpics(epics, callback) {
    ...
    // Create items list (MARKET:epic format for IG)
  ...
  /**
   * Handle incoming price update from Lightstreamer
   */
  _handlePriceUpdate(update) {
    try {
      const itemName = update.getItemName(); // MARKET:CS.D.EURUSD.CFD.IP
